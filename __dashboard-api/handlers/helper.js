...
const USERS_CONNECTIONS_TABLE_NAME = process.env.WS_CONNECTIONS_TABLE
// const USERS_CONNECTIONS_TABLE_NAME = 'websocket-users-connections'
const USERS_CONNECTIONS_CONNECTION_ID_GSI =
  'websocket-users-connections-connectionId-GSI'

module.exports.storeConnection = function createConnection({
  userId,
  connectionId,
}) {
  const params = {
...
module.exports.deleteConnection = function deleteConnection({ connectionId }) {
  ...
  return getConnectionDetailsByConnId({ connectionId }).then(deleteRecord)
}
...
module.exports.sendMessage = function sendMessage(event) {
  ...
  const addRecipientConnId = composedMessage => {
    return getConnectionDetailsByUserId({
      userId: composedMessage.recipientUserId,
  ...
  const sendMessageToUser = composedMessage => {
    return sendWSMessage({
      ...
      data: {
        fromUserId: composedMessage.senderUserId,
